...
public class TradingFixture : ScenarioFixture<TradingContext>
{
    ...
    public void HaveDynamicSharesOfMsftStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Buy, 
            Let["y"].As<string>(), Let["x"].As<int>(),
            new DateTime(2023, 1, 1, 10, 0, 0)));

    [Given("I have 100 shares of MSFT stock")]
    public void Have100SharesOfMsftStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Buy, 
            "MSFT", 100, new DateTime(2023, 1, 1, 10, 0, 0)));
    ...
    public void Have150SharesOfApplStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Buy,  "APPL", 150,
            new DateTime(2023, 1, 1, 10, 0, 0)));

    [When(@"Peço para vender :z ações da :y")]
    [When("I ask to sell :z shares of :y stock")]
    public void AskToSellDynamicSharesOfMsftStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Sell, Let["y"].As<string>(), Let["z"].As<int>(),
            new DateTime(2023, 1, 1, 10, 0, 0)));

    [When("I ask to sell 20 shares of MSFT stock")]
    public void AskToSell20SharesOfMsftStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Sell, "MSFT", 20,
            new DateTime(2023, 1, 1, 10, 0, 0)));

    [When("I ask to buy 20 shares of MSFT stock")]
    public void AskToBuy20SharesOfMsftStock() =>
        Context?.Trading.Execute(new TradingOrder(TradingType.Buy, "MSFT", 20,
            new DateTime(2023, 1, 1, 10, 0, 0)));
    ...
    public void ASellOrderFor20SharesOfMSFT() =>
        Context?.Trading.Book.GetOrderByType(TradingType.Sell, "MSFT", 20).Any().Should().BeTrue();
}