...
public static class TradingMethods
{
    public static ARRANGE I_have_100_shares_of_MSFT_stock(this ARRANGE fixtures) =>
        fixtures.Setup(f => f.Trading.Execute(new TradingOrder(TradingType.Buy, "MSFT", 100, new DateTime(2023, 1, 1, 10, 0, 0))));

    public static ARRANGE I_have_150_shares_of_APPL_stock(this ARRANGE fixtures) =>
        fixtures.Setup(f => f.Trading.Execute(new TradingOrder(TradingType.Buy, "APPL", 150, new DateTime(2023, 1, 1, 10, 0, 0))));

    public static ACT I_ask_to_sell_20_shares_of_MSFT_stock(this ACT fixtures) =>
        fixtures.It(f => f.Trading.Execute(new TradingOrder(TradingType.Sell, "MSFT", 20, new DateTime(2023, 1, 1, 10, 0, 0))));
    ...
    public static ASSERT A_sell_order_for_20_shares_of_MSFT_stock_should_have_been_executed(this ASSERT fixtures) =>
        fixtures.Expect(x => x.Trading.Book.GetOrderByType(TradingType.Sell, "MSFT", 20).Any().Should().BeTrue());
}